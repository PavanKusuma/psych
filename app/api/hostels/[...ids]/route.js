          ...
          if(params.ids[1] == 'U1'){
            ...
            // this is to get the count of HOSTEL students who are available for food on a given day.
                ...
                try {
                            ...
                            SELECT
                                ...
                                CAST(
                                    (
                                        (SELECT COUNT(*) FROM user WHERE type = 'hostel' AND role = 'student' and (year=1 or year=2 or year=3))
                                        - (SELECT COUNT(*) FROM request WHERE requestStatus = 'InOuting')
                                        + (SELECT COUNT(*) FROM request WHERE requestStatus = 'InOuting' AND DATE(requestTo) = "`+params.ids[2]+`" AND TIME(requestTo) < '22:00:00')
                    ...
                    return Response.json({status: 404, message:'No user found!'+error.message}, {status: 200})
                }
            }
            else {
